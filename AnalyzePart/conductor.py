...
class analyzeConductor:
    ...
    def InTheCaseOfUsertextParse(self, usertext:str, keyword_max:int = 3):
        resultAll = []
        ...
        for content in prase_list:
            ...
            score = content["score"]
            text = content["text"]
            resultDict = self.singleQueryAndAnalyze(text)
            resultAll.append({"keyword_score" : score, "result" : resultDict})
            indexnum += 1
        print("resultAll length : ", len(resultAll))
        return resultAll
...
if __name__ == "__main__":
    ...
    # userText = "アムウェイの木村です。\n後藤です。"
    userText = "実は今、私がとてもお世話になっている起業家の方が立ち上げた、仕組みからの収入が得られる\n案件がありまして、今メンバーの採用をしています。\n収入が別途、毎月安定的に入ってくる事業なのですが、詳細をご覧になりませんか？"
